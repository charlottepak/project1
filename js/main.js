...
let winner;
let playerDeck = [],
  dealerDeck = []; // arrays to hold seperate decks
let playerCards = [],
  dealerCards = []; // arrays to hold the cards dealt to them
let bet, bankroll;
let playerScore = 0;
let dealerScore = 0;
  ...
  for (var i = 0; i < suits.length; i++) {
    for (var x = 0; x < cardValues.length; x++) {
      var card = {
        ...
        suit: suits[i],
        cssClass: suits[i] + cardValues[x],
      };
      if (parseInt(cardValues[x])) {
        card.num = x + 1;
...
function dealCards() {
  ...
  dealerCards.forEach(function (card, i) {
    ...
    if (i === 0) {
      ...
      document.getElementById(
        "dealerCards"
      ).innerHTML += `<div class="card back"></div>`;
    }
  });
}
